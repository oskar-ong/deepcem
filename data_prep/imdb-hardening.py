...
COL_TCONST     = "tconst"
df_basics = pd.read_csv(PATH_RAW_TITLE_BASICS)
...
df_basics['cluster_id'] = df_basics[COL_TCONST].apply(lambda x: uf_movies.find(x))

# Mutation Rate
MUTATION_RATE = 0.2
# Dictionary where keys are cluster_ids and values are lists of entry names
mapping_m_orig_dups: Dict[str, List[str]] = df_basics.groupby('cluster_id')[COL_TCONST].apply(list).to_dict()
